...
class BodyThread(threading.Thread):
    ...
    def run(self):
        ...
        # 전체 실행 로직을 try-finally 블록으로 감싸서 자원 해제 보장
        try:
            self.setup_comms()
            ...
            with mp_pose.Pose(min_detection_confidence=0.80, min_tracking_confidence=0.5, model_complexity=global_vars.MODEL_COMPLEXITY, static_image_mode=False, enable_segmentation=True) as pose:
                ...
                if global_vars.KILL_THREADS or not capture.cap or not capture.cap.isOpened():
                    print("Camera or capture thread failed to start. Exiting BodyThread.")
                    return
            ...
            print(f"An error occurred in BodyThread: {e}")
            global_vars.KILL_THREADS = True

        finally:
            print("BodyThread is cleaning up resources...")
            ...
            # `capture.join()`을 호출하여 캡처 스레드가 완전히 종료되기를 기다립니다.
            if capture.is_alive():
                capture.join()

            cv2.destroyAllWindows()
            print("BodyThread has exited.")
    ...
    def setup_comms(self):
        if not global_vars.USE_LEGACY_PIPES:
            ...
            print("Using Named Pipes for interprocess communication (Windows only).")
            try:
